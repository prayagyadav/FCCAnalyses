...
Vec_rp getTwoHighestPMuons(Vec_rp muons) {
    if (muons.size() < 2) {
	...
	return Vec_rp(); // Return an empty vector if no valid pair is found
       }
...
Vec_rp getTwoHighestPtMuons(Vec_rp muons) {
    if (muons.size() < 2) {
        std::cerr << "Not enough muons to select the top 2." << std::endl;
        return {}; // Return an empty vector if there are fewer than 2 muons
    }
...
Vec_f coneIsolation0::coneIsolation0::operator() (Vec_rp in, Vec_rp rps) {
...
Vec_rp resonanceBuilder_mass::resonanceBuilder_mass::operator()(Vec_rp legs, Vec_i recind, Vec_i mcind, Vec_rp reco, Vec_mc mc, Vec_i parents, Vec_i daugthers) {
